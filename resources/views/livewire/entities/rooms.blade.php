    ...
    <form wire:submit.prevent="editRooms">
                    ...
                    @foreach ($rooms as $room)
                        <tr x-data="{ selected: false }">
                            <td
                                ...
                                <input @click="selected = ! selected" type="checkbox" wire:model.live="selectedRooms"
                                    value={{ $room->id }}>
                            </td>
                            @foreach ($tableProperties as $property)
                                <td
        ...
        <div class="bg-neutral-600 border border-neutral-400 p-3 flex justify-between items-center">
            ...
            <div class="flex gap-2">
                ...
                <button @click="showEditRoomsModal = ! showEditRoomsModal"
                    @if (count($selectedRooms) == 0) disabled @endif
            ...
            {{-- Edit Rooms Modal --}}
            <div class="absolute top-[2.5%] m-auto left-0 right-0 w-[50rem] rounded border border-neutral-600 z-50 bg-neutral-800 p-5"
                x-show="showEditRoomsModal" @click.outside="showEditRoomsModal = false">
                ...
                @livewire('entities.edit.show-edit-forms', ['rooms' => $selectedRooms])

            </div>

            {{-- Blur Background --}}
            <div x-show="showEditRoomsModal"
                class="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-xs transition-opacity">
            </div>
        </div>
    </form>
</div>
